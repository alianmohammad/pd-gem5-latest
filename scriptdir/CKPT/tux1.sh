...
# Test if the RUNSCRIPT_VAR1_TUX1 environment variable is already set
echo "***** Start tux1 ckpt script! *****"
if [ "${RUNSCRIPT_VAR1_TUX1+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR1_TUX1 not set! So setting up network. Then reload"\
            "self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR1_TUX1=1
else
...
if [ "$RUNSCRIPT_VAR1_TUX1" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR1_TUX1=2
	echo "3. RUNSCRIPT_VAR1_TUX1 is $RUNSCRIPT_VAR1_TUX1"
