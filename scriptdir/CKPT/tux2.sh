...
# Test if the RUNSCRIPT_VAR1_TUX2 environment variable is already set
echo "***** Start tux2 ckpt script! *****"
if [ "${RUNSCRIPT_VAR1_TUX2+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR1_TUX2 not set! So setting up network. Then reload"\
            "self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR1_TUX2=1
else
	echo "Else part - RUNSCRIPT_VAR1_TUX2 is set! So reload and execute!"
...
if [ "$RUNSCRIPT_VAR1_TUX2" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR1_TUX2=2
	echo "3. RUNSCRIPT_VAR1_TUX2 is $RUNSCRIPT_VAR1_TUX2"
