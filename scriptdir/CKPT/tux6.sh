...
# Test if the RUNSCRIPT_VAR1_TUX6 environment variable is already set
echo "***** Start tux3 ckpt script! *****"
if [ "${RUNSCRIPT_VAR1_TUX6+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR1_TUX6 not set! So setting up network. Then reload"\
            "self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR1_TUX6=1
else
	echo "Else part - RUNSCRIPT_VAR1_TUX6 is set! So reload and execute!"
...
ifconfig eth0 10.0.0.8
ifconfig lo up
ifconfig eth0 hw ether 00:90:00:00:00:07

################################################################################

#THIS IS WHERE EXECUTION BEGINS FROM AFTER RESTORING FROM CKPT
if [ "$RUNSCRIPT_VAR1_TUX6" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR1_TUX6=2
	echo "3. RUNSCRIPT_VAR1_TUX6 is $RUNSCRIPT_VAR1_TUX6"
