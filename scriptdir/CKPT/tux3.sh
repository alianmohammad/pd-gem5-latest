...
# Test if the RUNSCRIPT_VAR1_TUX3 environment variable is already set
echo "***** Start tux3 ckpt script! *****"
if [ "${RUNSCRIPT_VAR1_TUX3+set}" != set ]
then
	# Signal our future self that it's safe to continue
	echo "RUNSCRIPT_VAR1_TUX3 not set! So setting up network. Then reload"\
            "self, don't set up network and do this again n again n again n ..."
	export RUNSCRIPT_VAR1_TUX3=1
else
	echo "Else part - RUNSCRIPT_VAR1_TUX3 is set! So reload and execute!"
...
if [ "$RUNSCRIPT_VAR1_TUX3" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR1_TUX3=2
	echo "3. RUNSCRIPT_VAR1_TUX3 is $RUNSCRIPT_VAR1_TUX3"
