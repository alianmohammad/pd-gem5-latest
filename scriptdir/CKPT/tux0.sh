...
# make sure you rename RUNSCRIPT_VAR1_TUX0 to a different variable that was not
# defined in the script used to create the first ckpt.
################################################################################

# Test if the RUNSCRIPT_VAR1_TUX0 environment variable is already set
echo "***** Start tux0 ckpt script! *****"
if [ "${RUNSCRIPT_VAR1_TUX0+set}" != set ]
then
        # Signal our future self that it's safe to continue
        echo "RUNSCRIPT_VAR1_TUX0 not set! So setting it and taking checkpoint!"
        export RUNSCRIPT_VAR1_TUX0=1
else
...
# wait for other nodes to setup their network
sleep 0.1
...
if [ "$RUNSCRIPT_VAR1_TUX0" -eq 1 ]
then

	# Signal our future self not to recurse infinitely
	export RUNSCRIPT_VAR1_TUX0=2
	echo "3. RUNSCRIPT_VAR1_TUX0 is $RUNSCRIPT_VAR1_TUX0"
